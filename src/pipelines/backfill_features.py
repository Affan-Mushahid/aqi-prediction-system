...
def backfill_aqi_features(days=365):
    ...
    weather_df = pd.DataFrame({'datetime': pd.date_range(start=w_times[0], periods=len(w_times), freq='h')})
    ...
    weather_fc_df = pd.DataFrame({'datetime': pd.date_range(start=wf_times[0], periods=len(wf_times), freq='h')})
    for wf in weather_features:
    ...
    full_idx = pd.date_range(start=weather_all['datetime'].min(), end=weather_end, freq='h')
